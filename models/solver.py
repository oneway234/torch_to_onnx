...
from models.pit_criterion import cal_loss
        ...
        for i, (data) in enumerate(data_loader):
            ...
            if i % self.print_freq == 0:
                ...
                print('Epoch {0} | Iter {1} | Average Loss {2:.3f} | '
                      'Current Loss {3:.6f} | {4:.1f} ms/batch | {5:.6f}'.format(
                    epoch + 1, i + 1, total_loss / (i + 1),
                    loss.item(), 1000 * (time.time() - start) / (i + 1),
                    optim_state['param_groups'][0]['lr']), flush=True)

            if i % 300000 == 0:
                optim_state = self.optimizer.state_dict()
                optim_state['param_groups'][0]['lr'] = \
                    max(1e-6, optim_state['param_groups'][0]['lr'] * 0.50)
                self.optimizer.load_state_dict(optim_state)
                print('Learning rate adjusted to: {lr:.6f}'.format(
                    lr=optim_state['param_groups'][0]['lr']))
                
                file_path = os.path.join(
                    self.save_folder, 'epoch%d_%d.pth.tar' % (epoch + 1, i))
                torch.save(self.model.module.serialize(self.model.module,
                                                       self.optimizer, epoch + 1,
                                                       tr_loss=self.tr_loss,
                                                       cv_loss=self.cv_loss),
